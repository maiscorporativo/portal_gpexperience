...
const IS = { background: '#0d1f35', border: '1px solid #142030', borderRadius: 7, color: '#e8edf2', fontSize: 13, padding: '8px 10px', outline: 'none', width: '100%', boxSizing: 'border-box' as const };
...
function MasterLogin({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#060F1C', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <form onSubmit={handleSubmit} style={{ background: '#0a1628', border: '1px solid #142030', borderRadius: 16, padding: '40px 36px', width: 360, display: 'flex', flexDirection: 'column', gap: 16, boxShadow: '0 24px 64px rgba(0,0,0,.8)', animation: shake ? 'shake .4s ease' : 'none' }}>
        <div style={{ textAlign: 'center', marginBottom: 8 }}>
          <div style={{ width: 52, height: 52, background: 'linear-gradient(135deg, #f37126, #d45f1a)', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
            <Shield size={26} color="#000" />
          </div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#e8edf2', margin: 0 }}>Admin Mestre</h1>
          <p style={{ fontSize: 13, color: '#4a7fa8', marginTop: 6 }}>Painel de Aprovação — Zago</p>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <label style={{ fontSize: 11, color: '#4a7fa8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Usuário</label>
          <input type="text" value={username} onChange={e => { setUsername(e.target.value); setError(''); }} placeholder="Usuário" autoFocus autoComplete="username"
            style={{ background: '#0d1f35', border: `1px solid ${error ? '#7a1a1a' : '#142030'}`, borderRadius: 8, color: '#e8edf2', fontSize: 14, padding: '11px 14px', outline: 'none' }}
            onFocus={e => e.target.style.borderColor = '#f37126'} onBlur={e => e.target.style.borderColor = error ? '#7a1a1a' : '#142030'}
          />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <label style={{ fontSize: 11, color: '#4a7fa8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Senha</label>
        ...
        <button type="submit" disabled={loading} style={{ background: 'linear-gradient(135deg, #f37126, #d45f1a)', color: '#000', border: 'none', borderRadius: 8, padding: '12px', fontSize: 14, fontWeight: 700, cursor: loading ? 'wait' : 'pointer', opacity: loading ? 0.7 : 1 }}>
...
function MField({ label, icon, value, onChange, textarea = false, rows = 2 }: { label: string; icon?: React.ReactNode; value: string; onChange: (v: string) => void; textarea?: boolean; rows?: number }) {
  const focus = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => { e.target.style.borderColor = '#f37126'; };
  const blur =  (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => { e.target.style.borderColor = '#142030'; };
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <label style={{ fontSize: 10, color: '#4a7fa8', fontWeight: 600, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 4 }}>{icon}{label}</label>
      {textarea
...
function ImageUploadField({ label, labelIcon, value, onChange }: { label: string; labelIcon?: React.ReactNode; value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <label style={{ fontSize: 10, color: '#4a7fa8', fontWeight: 600, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 4 }}>{labelIcon}{label}</label>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <div style={{ flex: 1, display: 'flex', gap: 6 }}>
          <input value={value} onChange={e => onChange(e.target.value)} placeholder="https://..." style={{ ...IS, flex: 1 }}
            onFocus={e => e.target.style.borderColor = '#f37126'} onBlur={e => e.target.style.borderColor = '#142030'} />
          <input ref={fileRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={handleFile} />
          <button type="button" onClick={() => fileRef.current?.click()} title="Upload" style={{ padding: '8px 10px', background: '#0d1f35', border: '1px solid #142030', borderRadius: 7, color: '#f37126', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
            <Upload size={14} />
          </button>
        </div>
      </div>
...
function AuditTrail({ pkg }: { pkg: TrendingPackage }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 4 }}>
      {rows.map(r => (
        <span key={r.label} style={{ fontSize: 10, background: '#060f1c', border: `1px solid ${r.color}33`, borderRadius: 6, padding: '3px 8px', color: r.color, display: 'flex', gap: 4, alignItems: 'center' }}>
...
function PackageReviewCard({ pkg, index: _index, onApprove, onReject, onUpdate }: {
  ...
  const statusColor = pkg.status === 'approved' ? '#4ade80' : pkg.status === 'rejected' ? '#f87171' : '#ffb070';
  const statusBg    = pkg.status === 'approved' ? '#0d3320' : pkg.status === 'rejected' ? '#3a0d0d' : '#142030';
  const statusLabel = pkg.status === 'approved' ? '✅ Aprovado' : pkg.status === 'rejected' ? '❌ Rejeitado' : '🟡 Pendente';

  return (
    <div style={{ background: '#0a1628', border: `1px solid ${pkg.status === 'pending' || !pkg.status ? '#1a3a5c' : pkg.status === 'approved' ? '#1a5c38' : '#5c1a1a'}`, borderRadius: 12, overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', cursor: 'pointer' }} onClick={() => setOpen(o => !o)}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: '#4a7fa8', marginTop: 2 }}>{pkg.date} · {pkg.loc} · {pkg.currency || 'BRL'} {pkg.price}</div>
          <AuditTrail pkg={pkg} />
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {pkg.status !== 'approved' && (
      ...
      {/* Expanded edit form */}
      {open && (
        <div style={{ padding: '16px', borderTop: '1px solid #142030', display: 'flex', flexDirection: 'column', gap: 14 }}>
          <p style={{ fontSize: 11, color: '#4a7fa8', margin: 0 }}>{pkg.status === 'approved' ? 'Edite os campos e salve. O pacote continuará aprovado.' : 'Edite todos os campos necessários antes de aprovar:'}</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={{ fontSize: 10, color: '#4a7fa8', fontWeight: 600, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 4 }}><Globe2 size={11} /> Moeda</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, alignItems: 'end' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, paddingBottom: 2 }}>
              ...
              <button type="button" onClick={() => set({ isTrending: !local.isTrending })}
                style={{ width: 44, height: 24, borderRadius: 12, border: 'none', cursor: 'pointer', position: 'relative', transition: 'background .2s', background: local.isTrending ? '#f37126' : '#142030' }}>
              ...
              <span style={{ fontSize: 10, color: local.isTrending ? '#f37126' : '#4a7fa8' }}>{local.isTrending ? 'Sim' : 'Não'}</span>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', paddingTop: 4 }}>
            {pkg.status === 'approved' ? (
              <button onClick={handleSaveApprove} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', background: 'linear-gradient(135deg, #f37126, #d45f1a)', color: '#000', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                <Check size={14} /> Salvar Edições
              </button>
            ) : (
              <>
                <button onClick={handleSaveOnly} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', background: '#0d1f35', color: '#e8edf2', border: '1px solid #142030', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                  Salvar sem Aprovar
                </button>
                <button onClick={handleSaveApprove} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', background: 'linear-gradient(135deg, #f37126, #d45f1a)', color: '#000', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
...
/* ── Users Tab ── */
type AdminUser = { id: number; username: string; role: 'admin' | 'master' };
...
function UsersTab() {
  ...
  const [newUser, setNewUser] = useState({ username: '', password: '', role: 'admin' as 'admin' | 'master' });
  const [newErr, setNewErr] = useState('');
  const [newLoading, setNewLoading] = useState(false);

  // form state for edit
  const [editForm, setEditForm] = useState({ username: '', password: '', role: 'admin' as 'admin' | 'master' });
  ...
  const roleBg    = (r: string) => r === 'master' ? { background: '#2a1a00', color: '#f37126', border: '1px solid #7a3a00' } : { background: '#0d1f35', color: '#7bc4e8', border: '1px solid #1a3a5c' };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: '#4a7fa8', margin: '4px 0 0' }}>Gerencie quem tem acesso aos painéis Admin e Master.</p>
        </div>
        <button onClick={() => { setShowAdd(a => !a); setNewErr(''); }}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 14px', background: 'linear-gradient(135deg, #f37126, #d45f1a)', color: '#000', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
      ...
      {/* Add User Form */}
      {showAdd && (
        <form onSubmit={handleAdd} style={{ background: '#0a1628', border: '1px solid #f37126', borderRadius: 12, padding: '20px 20px 16px' }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: '#f37126', margin: '0 0 14px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>➕ Novo Usuário</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={{ fontSize: 10, color: '#4a7fa8', fontWeight: 600, textTransform: 'uppercase' }}>Usuário</label>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 14 }}>
            <label style={{ fontSize: 10, color: '#4a7fa8', fontWeight: 600, textTransform: 'uppercase' }}>Tipo de Acesso</label>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button type="button" onClick={() => setShowAdd(false)} style={{ padding: '8px 14px', background: 'transparent', color: '#4a7fa8', border: '1px solid #142030', borderRadius: 8, fontSize: 12, cursor: 'pointer' }}>Cancelar</button>
            <button type="submit" disabled={newLoading} style={{ padding: '8px 18px', background: 'linear-gradient(135deg, #f37126, #d45f1a)', color: '#000', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
      ...
      {/* User List */}
      {loading && <div style={{ textAlign: 'center', color: '#4a7fa8', padding: 32 }}>Carregando…</div>}
      ...
      {!loading && users.map(u => (
        <div key={u.id}>
          {/* Row */}
          {editId !== u.id ? (
            <div style={{ background: '#0a1628', border: '1px solid #142030', borderRadius: 12, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#0d1f35', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {u.role === 'master' ? <KeyRound size={16} color="#f37126" /> : <Users size={16} color="#7bc4e8" />}
              </div>
              <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => startEdit(u)}
                  style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', background: '#0d1f35', color: '#7bc4e8', border: '1px solid #1a3a5c', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            ...
            /* Edit Form */
            <form onSubmit={handleEdit} style={{ background: '#0a1628', border: '1px solid #7bc4e8', borderRadius: 12, padding: '16px 20px' }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: '#7bc4e8', margin: '0 0 12px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>✏️ Editando: {u.username}</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <label style={{ fontSize: 10, color: '#4a7fa8', fontWeight: 600, textTransform: 'uppercase' }}>Novo Usuário</label>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <button type="button" onClick={() => setEditId(null)} style={{ padding: '8px 14px', background: 'transparent', color: '#4a7fa8', border: '1px solid #142030', borderRadius: 8, fontSize: 12, cursor: 'pointer' }}>Cancelar</button>
                <button type="submit" disabled={editLoading} style={{ padding: '8px 18px', background: 'linear-gradient(135deg, #f37126, #d45f1a)', color: '#000', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
      ...
      {!loading && users.length === 0 && (
        <div style={{ textAlign: 'center', padding: '40px 20px', color: '#4a7fa8', fontSize: 14, background: '#0a1628', borderRadius: 12, border: '1px dashed #142030' }}>
...
export default function MasterAdmin() {
  ...
  useEffect(() => {
    if (saveError) {
      toast('Não foi possível salvar no servidor. Suas alterações estão seguras aqui — reinicie o servidor e tente novamente.', 'error');
    }
  }, [saveError, toast]);
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#060F1C', color: '#e8edf2', fontFamily: 'Inter, system-ui, sans-serif', alignItems: 'flex-start' }}>
      {/* Sidebar */}
      <aside style={{ width: 220, background: '#0a1628', borderRight: '1px solid #142030', display: 'flex', flexDirection: 'column', padding: '24px 16px', gap: 8, position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 8px 24px', fontSize: 15, fontWeight: 700, color: '#f37126', borderBottom: '1px solid #142030', marginBottom: 16 }}>
          <Shield size={18} /> Admin Mestre
        </div>

        {totalPending > 0 && (
          <div style={{ background: '#142030', border: '1px solid #1a3a5c', borderRadius: 8, padding: '10px 12px', fontSize: 12, color: '#7bc4e8', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {sections.map(s => (
          <button key={s.id} onClick={() => setSection(s.id)} style={{
            ...
            background: section === s.id ? '#142030' : 'transparent',
            color: section === s.id ? '#f37126' : '#4a7fa8',
          }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 7 }}>{s.icon} {s.label}</span>
            {s.count > 0 && <span style={{ fontSize: 11, background: '#1a3a5c', color: '#7bc4e8', padding: '1px 7px', borderRadius: 10, fontWeight: 700 }}>{s.count}</span>}
          </button>
        ))}

        <div style={{ borderTop: '1px solid #142030', margin: '8px 0' }} />

        {/* Users section */}
        <button onClick={() => setSection('users')} style={{
          display: 'flex', alignItems: 'center', gap: 8,
          padding: '10px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 13, fontWeight: 600,
          background: section === 'users' ? '#142030' : 'transparent',
          color: section === 'users' ? '#f37126' : '#4a7fa8',
        }}>
        ...
        <button onClick={logout} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderRadius: 8, border: '1px solid #142030', cursor: 'pointer', background: 'transparent', color: '#4a7fa8', fontSize: 12 }}>
          <LogOut size={14} /> Sair
        </button>
      </aside>
      ...
      <main style={{ flex: 1, padding: '32px', overflowY: 'auto' }}>
        ...
        {section === 'users' ? (
          ...
          <>
            ...
            <p style={{ fontSize: 13, color: '#4a7fa8', marginBottom: 24 }}>
            ...
            {curPkgs.length === 0 && (
              <div style={{ textAlign: 'center', padding: '60px 20px', color: '#4a7fa8', fontSize: 14 }}>
                {section === 'pending'
