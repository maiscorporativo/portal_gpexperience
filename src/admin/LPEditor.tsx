 ...
 * das seções da página (estrutura replicada da LP Indy 500):
 *
 *   Banco de Imagens → alimenta a seção "Galeria de Fotos" e fornece imagens
 *                       para as demais seções (Experiência, Destaque).
 *   Seção 1 — Topo da Página (Hero)
 *   Seção 2 — Cards de Benefícios
 *   Seção 3 — Programação do Evento
 *   Seção 4 — Pacotes & Tipos (ex: PACOTE PREMIUM / PACOTE VIP)
 *   Seção 5 — Experiência
 *   Seção 6 — Galeria de Fotos (gerada automaticamente pelo Banco de Imagens)
 *   Seção 7 — Destaque (opcional, estilo "Troféu Borg-Warner" da Indy 500)
 */
import React, { useState } from 'react';
import {
  Image as ImageIcon, Plus, Trash2, X, Video, LayoutGrid, CalendarDays,
  BedDouble, Star, Sparkles, Images, Loader2, Check, Tag, DollarSign, MapPin, Ticket,
} from 'lucide-react';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, background: '#0d0d0d', padding: 10, borderRadius: 10, border: '1px solid #1a1a1a' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 700, color: uploading ? '#555' : '#3b82f6', cursor: uploading ? 'wait' : 'pointer', padding: '3px 8px', background: 'rgba(59,130,246,0.1)', borderRadius: 6 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(96px, 1fr))', gap: 10 }}>
        ...
        <label style={{ aspectRatio: '1', borderRadius: 10, border: '1px dashed #333', background: '#0d0d0d', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: uploading ? 'wait' : 'pointer', color: '#3b82f6' }}>
      ...
      {bank.map((img, idx) => {
        ...
        return (
          <button key={`${idx}-${img}`} onClick={() => toggle(img)} title={isSel ? 'Clique para desmarcar' : 'Clique para usar esta imagem'}
            style={{ position: 'relative', aspectRatio: '1', borderRadius: 8, overflow: 'hidden', padding: 0, cursor: 'pointer', border: isSel ? '2px solid #4ade80' : '1px solid #2a2a2a', opacity: isSel ? 1 : 0.75 }}>
...
type OpcaoHospedagem = {
  selo?: string; nome?: string; subtitulo?: string; descricao_card?: string;
  valor_individual?: string; valor_duplo?: string; moeda?: string; parcelas?: string;
  nota_oferta?: string; inclusos?: Incluso[];
};
type PacotesObj = {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <LPSection badge="Seção 1 da LP" title="Topo da Página (Hero)" icon={Video} color="#e43c44"
        subtitle="Primeira dobra da LP: tag pequena em vermelho, título grande (Nome do Pacote), descrição e fundo em vídeo ou imagem.">
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <div style={fieldCol}>
            ...
            <textarea value={pkg.description || ''} onChange={e => onUpdate({ description: e.target.value })} rows={3}
              placeholder="Viva a emoção da corrida com um pacote completo: passagens aéreas, hospedagem e ingressos garantidos..." style={{ ...IS, resize: 'vertical' }} />
          </div>
        </div>
      </LPSection>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, padding: 12, background: '#0d0d0d', borderRadius: 10, border: '1px solid #1a1a1a' }}>
            <div style={fieldCol}>
              ...
              <input placeholder="Programação do" value={pkg.programacaoTitulo || ''} onChange={e => onUpdate({ programacaoTitulo: e.target.value })} style={IS} />
            </div>
            <div style={fieldCol}>
              <label style={lbl}>Título (parte vermelha)</label>
              <input placeholder="Fim de Semana" value={pkg.programacaoTituloDestaque || ''} onChange={e => onUpdate({ programacaoTituloDestaque: e.target.value })} style={IS} />
            </div>
            <div style={fieldCol}>
              <label style={lbl}>Subtítulo da seção</label>
              <input placeholder="Dias de ação e emoção" value={pkg.programacaoSubtitulo || ''} onChange={e => onUpdate({ programacaoSubtitulo: e.target.value })} style={IS} />
            </div>
          </div>
            ...
            <div key={i} style={{ background: '#0d0d0d', padding: 14, borderRadius: 10, border: '1px solid #1a1a1a' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr auto', gap: 8, marginBottom: 10 }}>
                ...
                <div style={fieldCol}>
                  ...
                  <input placeholder="Ex: Sexta, 28 de maio - Carb Day" value={day.titulo_dia || day.data || ''} onChange={e => { const n = [...prog]; n[i] = { ...n[i], titulo_dia: e.target.value }; setProg(n); }} style={IS} />
                </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {pacotes.opcoes_hospedagem.map((op, i) => (
            <div key={i} style={{ background: '#0d0d0d', padding: 16, borderRadius: 12, border: '1px solid #1a1a1a', display: 'grid', gap: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 10, fontWeight: 900, color: i === 0 ? '#e43c44' : '#fbbf24', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Card {i + 1} {i === 0 ? '(destaque vermelho)' : '(destaque dourado)'}</span>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div style={fieldCol}>
                  <label style={lbl}><Tag size={10} /> Tipo do Pacote (selo do card)</label>
                  <input placeholder="Ex: PACOTE PREMIUM" value={op.selo || ''} onChange={e => { const n = { ...pacotes }; n.opcoes_hospedagem[i] = { ...op, selo: e.target.value }; setPacotes(n); }} style={IS} />
                </div>
                <div style={fieldCol}>
                  <label style={lbl}>Nome do card (título)</label>
                  <input placeholder="Ex: O Espetáculo Indy 500" value={op.nome || ''} onChange={e => { const n = { ...pacotes }; n.opcoes_hospedagem[i] = { ...op, nome: e.target.value }; setPacotes(n); }} style={IS} />
                </div>
              </div>
              <div style={fieldCol}>
                <label style={lbl}>Subtítulo (frase abaixo do nome)</label>
                <input placeholder="Ex: A experiência completa para viver a maior corrida do mundo" value={op.subtitulo || ''} onChange={e => { const n = { ...pacotes }; n.opcoes_hospedagem[i] = { ...op, subtitulo: e.target.value }; setPacotes(n); }} style={IS} />
              </div>
              <div style={fieldCol}>
              ...
              <div style={fieldCol}>
                <label style={lbl}>Nota de oferta (caixa de aviso no rodapé do card)</label>
                <input placeholder="Ex: Apenas 10 vagas disponíveis para este pacote. Reserve agora!" value={op.nota_oferta || ''} onChange={e => { const n = { ...pacotes }; n.opcoes_hospedagem[i] = { ...op, nota_oferta: e.target.value }; setPacotes(n); }} style={IS} />
              </div>
          ...
          <button onClick={() => { const n = { ...pacotes }; n.opcoes_hospedagem = [...n.opcoes_hospedagem, { selo: '', nome: '', subtitulo: '', descricao_card: '', valor_individual: '', valor_duplo: '', moeda: 'USD', parcelas: '10', nota_oferta: '', inclusos: [] }]; setPacotes(n); }}
            style={{ alignSelf: 'flex-start', background: '#141414', border: '1px solid #2a2a2a', color: '#fff', padding: '6px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ padding: 14, background: 'linear-gradient(135deg, rgba(228,60,68,0.1), rgba(10,10,10,0.4))', borderRadius: 10, border: '1px solid #e43c44', display: 'grid', gap: 10 }}>
            <div style={{ ...lbl, color: '#e43c44' }}><Ticket size={11} /> Banner "Somente Ingresso" (opcional)</div>
      ...
      <LPSection badge="Seção 7 da LP · opcional" title="Seção Destaque" icon={Sparkles} color="#fbbf24"
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div style={fieldCol}>
              <label style={lbl}>Título (parte branca)</label>
              <input placeholder="Ex: O Troféu" value={destaque.titulo || ''} onChange={e => setDestaque({ ...destaque, titulo: e.target.value })} style={IS} />
            </div>
            <div style={fieldCol}>
              <label style={lbl}>Título (parte colorida)</label>
              <input placeholder="Ex: Borg-Warner" value={destaque.titulo_destaque || ''} onChange={e => setDestaque({ ...destaque, titulo_destaque: e.target.value })} style={IS} />
            </div>
          </div>
          <div style={fieldCol}>
            <label style={lbl}>Texto</label>
            <textarea rows={4} value={destaque.texto || ''} onChange={e => setDestaque({ ...destaque, texto: e.target.value })}
              placeholder="Conte a história ou destaque algo especial do evento..." style={{ ...IS, resize: 'vertical' }} />
          </div>
          <div style={fieldCol}>
            <label style={lbl}>Imagem da seção (escolha 1 do banco)</label>
            <BankPicker bank={bank} multiple={false} selected={destaque.imagem ? [destaque.imagem] : []}
              onChange={urls => setDestaque({ ...destaque, imagem: urls[0] || '' })} />
          </div>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#aaa', cursor: 'pointer' }}>
            <input type="checkbox" checked={!!destaque.invertido} onChange={e => setDestaque({ ...destaque, invertido: e.target.checked })} />
            Imagem à esquerda / texto à direita (invertido)
          </label>
        </div>
      </LPSection>
