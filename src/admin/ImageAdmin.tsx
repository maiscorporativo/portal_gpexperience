...
function AdminSection({ title, icon: Icon, children, color = '#e43c44' }: { title: string; icon: any; children: React.ReactNode; color?: string }) {
  return (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', cursor: 'pointer' }} onClick={handleToggle}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            {hasEdited && (
              <span style={{ fontSize: 10, background: '#3d2800', color: '#e43c44', border: '1px solid #7a4f00', padding: '2px 8px', borderRadius: 12, fontWeight: 700 }}>● Não salvo</span>
            )}
          </div>
          <div style={{ fontSize: 12, color: '#737373', marginTop: 2 }}>{pkg.date} · {pkg.loc} · {pkg.currency || 'BRL'} {pkg.price}</div>
        </div>
      ...
      {isOpen && isExternal && (
        <div style={{ padding: '0 16px 20px', borderTop: '1px solid #333333' }}>
          <div style={{ paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {trendMsg && (
              <div style={{ background: trendMsg.startsWith('🔥') ? '#0d3320' : '#1a2030', border: '1px solid #333333', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: trendMsg.startsWith('🔥') ? '#4ade80' : '#e43c44', fontWeight: 600 }}>
                {trendMsg}
              </div>
            )}
      ...
      {isOpen && !isExternal && (
        <div style={{ padding: '0 16px 20px', borderTop: '1px solid #333333' }}>
          <div style={{ paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  ...
                  <input
                    ...
                    onChange={e => set({ installments: e.target.value.replace(/\D/g, '').slice(0, 2) })}
            ...
            <LPContentEditor pkg={local} onUpdate={set} tokenKey="emais_admin_token" />
            ...
            <div style={{
              ...
              border: `1px solid ${hasEdited ? '#e43c44' : '#333333'}`, borderRadius: 12,
              display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12,
            }}>
              <span style={{ fontSize: 12, color: hasEdited ? '#e43c44' : '#737373', fontWeight: 600 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: hasEdited ? '#e43c44' : '#333333',
                  color: hasEdited ? '#fff' : '#737373', border: 'none', borderRadius: 8,
