...
/**
 * Migração automática de dados (roda no startup do GP): se a tabela
 * compartilhada ainda não tem pacotes de origem 'gp', copia os pacotes
 * ativos do site_content para lá — mesma lógica (e mesmas garantias de
 * idempotência) do script server/migrate-shared-packages.js.
 */
export async function migrateGpPackagesIfEmpty(pool) {
  if (!sharedDbEnabled()) return;
  const [existing] = await sharedPool.query(
    "SELECT COUNT(*) AS c FROM shared_packages WHERE origem = 'gp'"
  );
  if (existing[0].c > 0) return; // já migrado

  const [rows] = await pool.query('SELECT packages FROM site_content WHERE id = 1');
  if (!rows.length) return;
  let packages = [];
  try { packages = JSON.parse(rows[0].packages || '[]'); } catch { return; }
  const ativos = packages.filter(p => p && !p.deletedAt);
  ...
  for (let i = 0; i < ativos.length; i++) {
    ...
    await sharedPool.query(
      ...
      [JSON.stringify(pkg), pkg.isTrending === true ? 1 : 0, i, i, i]
    );
  }
  console.log(`✅ Migração automática: ${ativos.length} pacote(s) do GP copiados para a tabela compartilhada.`);
}
